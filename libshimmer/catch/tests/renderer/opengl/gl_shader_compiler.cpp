            ...
            "[gl_shader_compiler]" )
{
    ...
    // Temporarily disable logging to avoid spamming the console with errors.
    spdlog::set_level ( spdlog::level::off );

    auto swapped_fs = gl_shader_compiler::compile (
                          simple_vs_source,
                          GL_FRAGMENT_SHADER );

    auto swapped_vs = gl_shader_compiler::compile (
                          simple_fs_source,
                          GL_VERTEX_SHADER );

    auto garbage_fs = gl_shader_compiler::compile (
                          garbage_source,
                          GL_FRAGMENT_SHADER );

    auto garbage_vs = gl_shader_compiler::compile (
                          garbage_source,
                          GL_VERTEX_SHADER );

    auto empty_fs = gl_shader_compiler::compile (
                        empty_source,
                        GL_FRAGMENT_SHADER );

    auto empty_vs = gl_shader_compiler::compile (
                        empty_source,
                        GL_VERTEX_SHADER );
